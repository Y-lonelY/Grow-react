...
import {
  getDailyExerciseList,
  addExerciseList,
  getGoalList,
} from '@/service/practice/service'
...
class DailyView extends React.PureComponent<ExerciseProps, ExerciseState> {
    ...
    const sumListView = Object.entries(sumMap).map((item, index) => {
      return (
        <Row className="dailySumItem" key={index}>
          <Col
            className="dailySumTitle"
            span={8}
            style={{ color: colors[index] }}
          >
            {String(item[0]).toUpperCase()}
          </Col>
          <Col className="dailySumLabel" span={16}>
            {item[1] ? item[1] : '-'}
          </Col>
        </Row>
      )
    ...
    if (locale === 'zh_cn') {
      ...
      title =
        this.state.chart.length > 0
          ? `共${this.state.chart.length}条锻炼记录`
          : ''
    } else {
      title =
        this.state.chart.length > 0
          ? `total ${this.state.chart.length} records`
          : ''
    }
                ...
                <Pie
                  data={
                    this.props.exerciseData
                      ? this.props.exerciseData.sumMap
                      : {}
                  }
                ></Pie>
              </div>
   ...
   */
  public async update(
    params: QueryInterface = this.params,
  ...
  handlePolylineData: () => {
    ...
    const chartList: PolylineData[] = []
    const tableList: ExerciseTableData[] = []
    const avgData: PieData = {}

    // 处理列表数据
    if (
      this.props.exerciseData &&
      this.props.exerciseData.dailyList &&
      this.props.exerciseData.dailyList.length > 0
    ) {
      ...
      // 平均值获取
      avgData['leg'] = this.props.exerciseData.sumMap.leg
        ? Number(this.props.exerciseData.sumMap.leg) / len
        : 0
      avgData['belly'] = this.props.exerciseData.sumMap.belly
        ? Number(this.props.exerciseData.sumMap.belly) / len
        : 0
      avgData['chest'] = this.props.exerciseData.sumMap.chest
        ? Number(this.props.exerciseData.sumMap.chest) / len
        : 0
    }
