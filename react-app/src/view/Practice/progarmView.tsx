...
class ProgramView extends React.PureComponent<
  ProgramOverviewProps,
  programOverviewState
> {
    ...
    if (locale === 'zh_cn') {
      title =
        this.state.list.length > 0
          ? `共${this.state.list.length}条编程${
              this.state.type === 'lang' ? '语言' : '项目'
            }记录`
          : ''
    } else {
      title =
        this.state.list.length > 0
          ? `total ${this.state.list.length} ${
              this.state.type === 'lang' ? 'language' : 'project'
            } records`
          : ''
    }
    return (
        ...
        <Row>
          <Col className="listView" span={18}>
            <ChartBar
              title={title}
              defaultDateRange={[
                moment(this.params.start),
                moment(this.params.end),
              ]}
          ...
          <Col span={6}>
            <div className="sumView">
              {Array.isArray(
                this.props.programOverviewData[this.state.type].list
              ) &&
              this.props.programOverviewData[this.state.type].list.length >
                0 ? (
                <List
                  size="small"
  ...
  getDisplayList() {
    ...
    // 优先排序，保证色值渐变
    const sumList = Object.entries(data)
      .sort((a, b) => {
        return Number(b[1]) - Number(a[1])
      })
      .map((item, index) => {
        const desc =
          `${String(item[1]).replace(/(\d)(?=(?:\d{3})+$)/g, '$1,')}-` +
