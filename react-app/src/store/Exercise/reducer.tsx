 ...
 */
const defaultExerciseState: TS.ExerciseData = {
  dailyList: [],
  sumMap: {},
}

export const exerciseData: (
  state: TS.ExerciseData,
  action: TS.ExerciseChartAction
) => TS.ExerciseData = (state = defaultExerciseState, action) => {
  switch (action.type) {
 ...
 * goal list module
 */
const defaultGoalListState: TS.GoalListItem[] = []

export const goalListData: (
  state: TS.GoalListItem[],
  action: TS.GoalListAction
) => TS.GoalListItem[] = (state = defaultGoalListState, action) => {
  switch (action.type) {
 ...
 * program module
 */
const defaultProgramState: TS.ProgramOverviewTemplate = {
  lang: {
    list: [],
    name: [],
  },
...
export const programOverviewData = (state = defaultProgramState, action) => {
  if (action.type === TS.ExerciseTypes.PROGRAMOVERVIEW) {
    return { ...state, lang: action.lang, project: action.project }
  } else {
    return state
  }
}
